...
export default defineEventHandler(async (event) => {
  ...
  const citas = await controller
    .setModel(Citas)
    .setWhereFilters({ idCliente: id as string, estatus: "sala_espera" })
    .setIncludedModels([
      {
        model: Horarios,
        as: "horarioCita",
    ...
    .getAll()
    .then((res) =>
      res.toRawArray<{ horarioCita: Horarios; servicioCita: Servicios }>()
    )
    .catch((err) => {
      throw createError({
        statusCode: 400,
  ...
  const cliente = await controller
    .setModel(Clientes)
    .setWhereFilters({ idCliente: id as string })
    .setAttributes(["nombre", "apellidos", "nombreCompleto", "numTelefono"])
    .getModelResult()
    .getOne()
